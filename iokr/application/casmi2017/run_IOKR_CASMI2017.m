...
%     load (filename);    
    ...
    for challenge_idx = challenge_ids
        ...
        for mf_cand_id = 1:5
            ...
            [scores_test, ~, hh_test] = Test_IOKR (KX_list_train_test, KX_list_test, train_model, ...
                Y_train, Y_C_test, iokr_param.iokr_param.center);

            result_test = [result_test, table( ...
                scores_test{1}', ... 
                repmat (hh_test, [numel(scores_test{1}), 1]), ...
                'VariableNames', {sprintf('score_%02d', mf_cand_id), sprintf('hh_%02d', mf_cand_id)})];
        end % for
            
        writetable (result_test, sprintf ('%s/%s/iokr-category4-%03d_with_hh.txt', ...
