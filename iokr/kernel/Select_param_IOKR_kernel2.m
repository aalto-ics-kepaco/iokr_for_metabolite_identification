function [ lambda_opt, KY_par_opt, w_opt ] = Select_param_IOKR_kernel2( KX_list_train, Y_train, KY_opt, param, param_grid )
%======================================================
% DESCRIPTION:
...
%======================================================    
    ...
    switch KY_opt.type
        ...
        case {'linear','tanimoto'}
            val_KY_param = 1; % this value will not be used
            
        case 'gaussian'
            switch KY_opt.param_selection
                case 'mse'
                    if isfield(param_grid, 'gamma')
                        val_KY_param = param_grid.gamma;
                    else
                        val_KY_param = [0.001 0.005 0.01 0.05 0.1 1];
                    end
                case 'entropy'
                val_KY_param = select_gamma_entropy(Y_train, KY_opt.base_kernel);
            end
            
        case 'polynomial'

            if isfield(param_grid, 'offset')
                val_KY_param1 = param_grid.offset;
            else
                val_KY_param1 = [0 1e-3 1e-2 1e-1 1];
            end

            if isfield(param_grid, 'degree')
                val_KY_param2 = param_grid.degree;
            else
