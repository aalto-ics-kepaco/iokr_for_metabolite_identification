function runtime_measurements (inputDir, param, algorithm)
...
%   - ELSE  calcuate: cv-partition, selection and statistic
    ...
    if (nargin < 1)
        error ('runtime_measurements:InvalidInput', ...
    ...
    if (~ exist (inputDir, 'dir'))
        error ('runtime_measurements:InvalidInput', '%s: No such directory.', ...
            inputDir);
    end % if
    if (~ any (strcmp (algorithm, {'IOKR', 'MP-IOKR'})))
        error ('runtime_measurements:InvalidInput', 'Invalid algorithm to measure: %s', ...
            algorithm);
    end % if
    ...
    fprintf ('Algorithm: %s\n', upper (algorithm));
    fprintf ('REV_IOKR: %s\n', upper (param.mp_iokr_param.rev_iokr));    
    ...
    swc_prediction              = StopWatchCPUTime ('Prediction (CPU)');
    ...
    sw_prediction               = StopWatch ('Prediction (WALL)');
    ...
    n_kx = numel (KX_list);
    fprintf ('Number of input-kernels: %d\n', n_kx);
    ...
    switch (upper (algorithm))
        case 'IOKR' 
            swc_prediction.start();
            sw_prediction.start();
            
            Psi_pred = Prediction_IOKR_feat (KX_train_test_list{1}, C);
            
            swc_prediction.stop();
            swc_prediction.showAvgTime();
            sw_prediction.stop();
            sw_prediction.showAvgTime();
